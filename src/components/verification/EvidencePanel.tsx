  ...
  return (
    <section className={cn("rounded-2xl border border-slate-200 bg-white p-5 shadow-tech", className)}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-lg border border-amber-300 bg-amber-50 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-amber-700">
          Demo Evidence
        </span>
      </div>
...
function UploadBlock({ photo, error, inputRef, propertyId, onPick, onReplace, onRemove, onFiles }: UploadBlockProps) {
  return (
    <>
      <p className="mb-1.5 mt-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Demo Evidence Upload</p>
      {photo ? (
        <div className="overflow-hidden rounded-xl border border-slate-200">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={photo.url} alt={`Demo field evidence for ${propertyId}`} className="h-40 w-full bg-slate-950 object-cover" />
